...
if (empty($user_data['gizli_soru'])) {
    header('Location: profil-guncelle.php');
    exit;
}
?>
...
<body>
    ...
    <a href="ihtiyac.php"><button class="button">İhtiyaçları Gör</button></a>
    </div>
</center>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // --- 7. DIŞARIDAN GELEN URL PARAMETRESİNİ KONTROL ETME ---
    const urlParams = new URLSearchParams(window.location.search);
    
    // Eğer URL'de ?sohbet=yapayzeka varsa
    if (urlParams.get('sohbet') === 'yapayzeka') {
        // 1. Destek butonuna tıkla (pencereyi aç)
        chatToggleBtn.click();
        
        // 2. Asistan sekmesine tıkla (Canlı destek açık kalmışsa diye garantiye alıyoruz)
        const asistanButonu = document.querySelector('.tab-btn[data-target="bot-tab"]');
        if (asistanButonu) {
            asistanButonu.click();
        }

        // 3. (Opsiyonel ama Profesyonel) URL'yi temizle.
        // Kullanıcı sayfayı yenilerse (F5) chat penceresi tekrar zorla açılmasın diye URL'deki '?sohbet=yapayzeka' yazısını sileriz.
